...
restore_from_container() {
  ...
  for path in "${media_dirs[@]}"; do
    if [[ "$path" == templates ]]; then
      remove_paths+=("/app/templates")
    else
      remove_paths+=("/app/${path}")
    fi
  done
  local remove_command
  printf -v remove_command 'rm -rf %q ' "${remove_paths[@]}"
  ${PODMAN_BIN} exec -i "${APP_CONTAINER_NAME}" sh -lc "$remove_command"
